...
var SuggestService = (function () {
    ...
    SuggestService.prototype.hoverProvider = function (document, position) {
        ...
        var wordStartIndex = line.text.slice(0, position.character).search(/[a-z0-9_]+$/i);
        var wordEndIndex = line.text.slice(position.character).search(/[^a-z0-9_]/i) + position.character;
        var word = line.text.slice(wordStartIndex, wordEndIndex);
        if (!word) {
            return null;
        }
        ...
        var commandArgs = [position.line + 1, wordEndIndex, document.fileName, this.tmpFile];
        return this.runCommand(document, 'complete-with-snippet', commandArgs).then(function (lines) {
            ...
            var match = parts[1];
            var type = parts[6];
            var definition = parts[7];
            var docs = JSON.parse(parts[8].replace(/\\'/g, "'")).split('\n');
            ...
            for (var i = 0; i < docs.length; i++) {
                ...
                if (docLine.trim().startsWith('#')) {
                    var headerMarkupEnd = docLine.trim().search(/[^# ]/);
    ...
    SuggestService.prototype.dataHandler = function (data) {
        ...
        var lines = (this.dataBuffer + dataStr).split(/\r?\n/);
        this.dataBuffer = '';
