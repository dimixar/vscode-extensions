...
var CargoTask = (function () {
    ...
    CargoTask.prototype.execute = function (cwd) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var customEnv = vscode.workspace.getConfiguration('rust')['cargoEnv'];
            if (customEnv) {
...
var CommandService = (function () {
    ...
    CommandService.parseNewHumanReadable = function (errors, output) {
        var newErrorRegex = new RegExp('(warning|error|note|help)(?:\\[(.*)\\])?\\: (.*)\\s+--> '
            + '(.*):(\\d+):(\\d+)\\n(?:((?:.+\\n)+)\\.+)?(?:[\\d\\s]+\\|.*)*\\n((?:\\s+=.*)+)?', 'g');
        var newErrorRange = /\s+\|\s+(\^+)/g;
        while (true) {
            var match = newErrorRegex.exec(output);
            var range = newErrorRange.exec(output);
            if (match == null) {
                break;
            }
